...
class EventCreateRequest implements IAbstractWebDAVRequest
{
    ...
    public function getContent()
    {
        ...
        if(!empty($this->vo->getLocationName())){
            $event->setLocation($this->vo->getLocationName());
        }

        if(!empty($this->vo->getFreq())){
            $recurringRule = new RecurrenceRule();
            $recurringRule->setFreq($this->vo->getFreq());
            $recurringRule->setUntil($this->vo->getUntil());
            $recurringRule->setByDay($this->vo->getByDay());
